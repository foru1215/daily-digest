...
> **Note:** Fine-grained PAT は Projects v2 の読み取り権限に対応していないため、Classic PAT を使用します。

1. https://github.com/settings/tokens/new にアクセス
2. 設定:
   - **Note:** `daily-digest`（任意の名前）
   - **Expiration:** 90 days（定期的に更新）
   - **Scopes:**
     - ✅ `repo`（Issues の読み書き用）
     - ✅ `read:project`（Projects v2 の読み取り用）
...
| `Resource not accessible` | PAT の権限不足。Classic PAT で `repo` + `read:project` を確認 |
